...
def get_SESiL_config(parser, env_name):
    ...
    parser.add_argument("--evo_pretrain_mode", type=str, default="common_head", choices=["none", "encoder", "full", "common", "common_head"], help="pretrain mode: 'none'=each solver trains on assigned tasks, 'encoder'=train one shared encoder then copy+freeze encoder and finetune heads, 'full'=train each solver via RL on all tasks, 'common'=train one solver on all tasks then copy+freeze full actor and finetune critic, 'common_head'=like common but copies actor+critic, wipes last layer, finetunes only the last layer")
