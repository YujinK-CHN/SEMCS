...
def get_SESiL_config(parser, env_name):
    ...
    parser.add_argument("--evo_pretrain_mode", type=str, default="none", choices=["none", "encoder", "full", "common"], help="pretrain mode: 'none'=each solver trains on assigned tasks, 'encoder'=train one shared encoder via RL on all tasks then copy to all solvers, 'full'=train each solver via RL on all tasks, 'common'=masked entity modeling (self-supervised) on all tasks then copy encoder to all solvers")
    parser.add_argument("--evo_mask_ratio", type=float, default=0.15, help="fraction of entities to mask during common pretrain")
