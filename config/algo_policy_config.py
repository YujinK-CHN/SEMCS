...
def get_SESiL_config(parser, env_name):
    ...
    parser.add_argument("--evo_pretrain_mode", type=str, default="none", choices=["none", "encoder", "full"], help="pretrain mode: 'none'=each solver trains on assigned tasks, 'encoder'=train one shared encoder via RL on all tasks then copy to all solvers, 'full'=train each solver via RL on all tasks")
