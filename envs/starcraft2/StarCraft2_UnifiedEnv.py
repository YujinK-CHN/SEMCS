...
from utils.port_picker import pick_unique_port
...
class StarCraft2UnifiedEnv(MultiAgentEnv):
    ...
    def __init__(
        ...
        map_params = get_smac_params(self.map_name)
        ...
        # set a uniqe port for the parallel environments to avoid port conflict
        self._port = pick_unique_port() if port is None else port
        
        # set file dir
        self.tmp_dir = f"/tmp/sc2_temp_{uuid.uuid4().hex}"
        ...
        for i in range(self.n_agents):
            ...
            if "scvd" in self.args.algorithm_name:
                state_size = self.get_obs_size()
                state_size[0] = (state_size[0]-4) * self.n_agents  # remove movement features
                self.share_observation_space.append(state_size)
            else:
                self.share_observation_space.append(self.get_state_size())

        if not self.use_single_state and self.use_stacked_frames:
    ...
    def _launch(self):        
        # time.sleep(random.uniform(0.2, 1.0))  # stagger to avoid simultaneous SC2 launches
        try:
            """Launch the StarCraft II game."""
            self._run_config = run_configs.get(version=self.game_version)
            if self._run_config.tmp_dir is None:
                self._run_config.tmp_dir = self.tmp_dir
            _map = maps.get(self.map_name)
            self._seed += 1
            # Setting up the interface
            interface_options = sc_pb.InterfaceOptions(raw=True, score=False)
            self._sc2_proc = self._run_config.start(window_size=self.window_size, want_rgb=False, stderr=subprocess.DEVNULL, host="127.0.0.1", port=self._port)
            ...
            print(f"[Error PID {os.getpid()}] SC2 failed to launch or join game: {e}")
            self.close()  # clean up the controller
            raise RuntimeError("SC2 launch failed") from e
    ...
    def reset(self):
        ...
        if self.use_single_state:
            global_state = self.get_single_state()
        else:
        ...
        if "scvd" in self.args.algorithm_name:
            global_state = np.concatenate(local_obs, axis=0)     
    ...
    def _restart(self):
        ...
        try:
            ...
            print("Error:", protocol.ProtocolError, protocol.ConnectionError)
            self.full_restart()
    ...
    def step(self, actions):
        ...
        # print("actions:", actions, [self.get_avail_agent_actions(i) for i in range(self.n_agents)])
    ...
    def get_obs_agent(self, agent_id):
        ...
        move_feats_dim = self.get_obs_move_feats_size()  # 4 + 2
        ...
        # print("feats_dim:", move_feats_dim, enemy_feats_dim, ally_feats_dim, own_feats_dim)
    ...
    def get_obs(self):
        ...
        # transfer
        agents_obs = ft.translate_local_obs(agents_obs, self.args.use_map_ohid, self.args.algorithm_name)
        return agents_obs
    ...
    def get_obs_size(self):
        ...
        own_feats = self.get_obs_own_feats_size() # 4 + unit_type...
        move_feats = self.get_obs_move_feats_size() # 4

        n_enemies, n_enemy_feats = self.get_obs_enemy_feats_size()  # 4
        n_allies, n_ally_feats = self.get_obs_ally_feats_size()  # 4

        enemy_feats = n_enemies * n_enemy_feats
        ally_feats = n_allies * n_ally_feats

        # e.g., in 3m, we have, 4 + 4 * 3 + 4 * 2 + 4 = 28
        all_feats = move_feats + enemy_feats + ally_feats + own_feats

        agent_id_feats = 0
        timestep_feats = 0

        # 28 + 4 for observing ids
        if self.obs_agent_id:
    ...
    def close(self):
        ...
        try:
            ...
            print(f"[Exception in Close] try to delete {self.tmp_dir}: {e}")
