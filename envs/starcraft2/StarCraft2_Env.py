...
class StarCraft2Env(MultiAgentEnv):
    ...
    def __init__(
        ...
        self.last_action_short = None
        self.last_full_action_short = None
        ...
        for i in range(self.n_agents):
            ...
            if "maic" in self.args.algorithm_name or "tgcnet" in self.args.algorithm_name:
                state_size = self.get_state_size()
                state_size[0] = state_size[0] * self.n_agents  # remove movement features
    ...
    def step(self, actions, full_actions=None):
        ...
        #############################################
        # prevent unavailable actions
        actions_int = []
        for i in range(self.n_agents):
            avail_act = self.get_avail_agent_actions(i)
            if avail_act[int(actions[i])] != 1:
                # use a random available action
                actions_int.append(self._prevent_unavail_acts(avail_act))  # change unavailable actions to stop
            else:
                actions_int.append(int(actions[i]))
        # actions_int = [int(i) for i in actions]
        #############################################
        
        self.last_action_short = actions
        self.last_full_action_short = full_actions
        ...
        if ally:  # use new SC2 unit types
            if self.map_type == "MM_queen":
                ...
                if unit.unit_type == 1972:
					...
					# overload
                    type_id = 1
                elif unit.unit_type == 2007:
                    type_id = 0
                elif unit.unit_type == 2006:
                    type_id = 1
                else:
                    type_id = 2
            elif self.map_type == "overload_bane":
            ...
            if self.map_type == "stalkers_and_zealots":
				...
				# zergling
                type_id = 0
        # print("map_type:", self.map_type, ally, unit.unit_type)
    ...
    def _prevent_unavail_acts(self, actions):
    # Find indices where the value is 1
        indices_with_1 = np.where(np.array(actions) == 1)[0]
        if indices_with_1.size > 0:
            return np.random.choice(indices_with_1)
        else:
            return 1
