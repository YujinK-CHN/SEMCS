...
class ACTLayer(nn.Module):
    ...
    def forward(self, x, available_actions=None, deterministic=False, n_actions=0):
        ...
        # check_actions = np.all([available_actions[i, action].numpy() for i, action in enumerate(actions)])
        # # print("Check available actions:", check_actions)
        # if not check_actions:
        #     print("Unavailable actions:", actions, [available_actions[i, action] for i, action in enumerate(actions)])
    ...
    def evaluate_actions(self, x, action, available_actions=None, active_masks=None, deterministic=False):
        ...
        return actions, action_log_probs, dist_entropy, action_logits.probs
