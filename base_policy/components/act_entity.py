...
class EntityVAECategorical(nn.Module):
    ...
    def forward_i(self, x, available_actions=None, n_enemies=None):
        ...
        # action_value = self.linear(torch.cat([torch.mean(x, -2, True), x[:, -n_enemies:]], dim=-2))
...
class EntityVAEACTLayer(nn.Module):
    ...
    def forward4skills(self, x, available_actions=None, deterministic=False, n_enemies=None):
        ...
        # # clip the values
        # pi_list = [torch.clamp(pi, min=0.0001, max=0.9999) for pi in pi_list]
        # pi_list = [pi/(pi.sum(-1).unsqueeze(-1)) for pi in pi_list]
