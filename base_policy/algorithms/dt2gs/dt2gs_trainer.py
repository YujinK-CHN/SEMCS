...
class dt2gsTrainer():
    """
    Trainer class for DT2GS to update policies, which can be based on IPPO or MAPPO.
    Reuses mcsTrainer's PPO + VAE-loss logic: main.py forces args.skill_choice
    to "UseVAE" and args.num_skills = args.num_subtask for dt2gs, so the
    subtask context-encoder losses flow through the same "skill_choice == UseVAE"
    branch used by mcs.
