"""
SESiL actor and critic.

Actor: flat obs → zero-pad → MLP encoder (per-task, evolves) → shared act_layer
Critic: reuses IntegrationCritic (entity obs, shared across methods)

Each solver in the population is a full actor. The encoder is the main
computation; the act_layer is a thin shared head.
"""
import torch
...
class R_Actor(nn.Module):
    ...
    def forward(self, obs, rnn_states, rnn_states_comm, masks, active_masks, available_actions=None,
        ...
        fea_list = [self.encoder(ob) for ob in flat_obs_list]
